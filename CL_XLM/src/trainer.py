...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        # how many time has been seen for each sample by our model
        self.src_seentimes = np.ones((1000000,), dtype=np.float)
        self.tgt_seentimes = np.ones((1000000,), dtype=np.float)
        ...
        self.stats = OrderedDict(
            ...
            [('Original_AE-%s' % lang, []) for lang in params.ae_steps] +
            [('MT-%s-%s' % (l1, l2), []) for l1, l2 in params.mt_steps] +
            [('BT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps] +
            [('Original_BT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps]
        )
    ...
    def checkpoint_paths(self, path, pattern=r"periodic-(\d+)\.pth"):
    ...
    def save_periodic(self):
        ...
        if self.params.keep_last_epochs > 0:
            ...
            checkpoints = self.checkpoint_paths(self.params.dump_path, pattern=r"periodic-(\d+)\.pth")
            for old_chk in checkpoints[self.params.keep_last_epochs:]:
...
class EncDecTrainer(Trainer):
    ...
    def comput_js_distance(self, p, q):
        log_pq_mean = torch.log((p + q) / 2.0)
        return 0.5 * self.KLDiv(log_pq_mean, p) + 0.5 * self.KLDiv(log_pq_mean, q)

    def get_token_distribution(self, x1, len1, langs1, x2, len2, langs2, pred_mask, y, masked_y):
        with torch.no_grad():
            # encode source sentence and translate it at the training mode for the first time
            train_enc = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False)
            train_enc = train_enc.transpose(0, 1)

            # decode original sentence
            sample_dec = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=train_enc,
                                      src_len=len1)

            (scores, _, _), _ = self.decoder('predict', tensor=sample_dec, pred_mask=pred_mask, y=y, masked_y=masked_y,
                                             get_scores=True)
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        weight_pred_mask = alen[:, None] >= len2[None] - 1  # mask for weights
        y = x2[1:]
        masked_y = x2[1:].masked_select(pred_mask[:-1])
        assert len(masked_y) == (len2 - 1).sum().item()

        # cuda
        x1, len1, langs1, x2, len2, langs2, y, masked_y = to_cuda(x1, len1, langs1, x2, len2, langs2, y, masked_y)
        ...
        if self.params.ae_js == True:
            # compute the distribution of tokens for 2 times in no_grad mode
            distribution_1 = self.get_token_distribution(x1, len1, langs1, x2, len2, langs2, pred_mask, y, masked_y)
            distribution_2 = self.get_token_distribution(x1, len1, langs1, x2, len2, langs2, pred_mask, y, masked_y)
            # compute JS distance
            token_distance = self.comput_js_distance(distribution_1, distribution_2)
            token_distance = token_distance.sum(dim=-1)

            token_distance = token_distance.view_as(y)  # (len - 1, bs)
            sentence_distance = (token_distance * pred_mask[:-1].float().cuda()).sum(dim=0) / (
                        len2 - torch.ones_like(len2))

            # normalize token distence from 0 to 1
            token_distance_max = token_distance.max(dim=0)[0]
            token_distance_min = token_distance.min(dim=0)[0]
            token_distance = (token_distance - token_distance_min) / \
                             (token_distance_max - token_distance_min + 1e-6 * torch.ones_like(token_distance_min))

            # compute weight at token level and sentence level
            token_weight = (torch.ones_like(token_distance) - token_distance) ** self.params.exp_param
            token_weight = token_weight.masked_fill(mask=weight_pred_mask[:-1].cuda(), value=float("-inf"))
            token_weight = torch.softmax(token_weight, dim=0)

            # compute sentence weight and normalize it
            sentence_distance_max = sentence_distance.max(dim=0)[0]
            sentence_distance_min = sentence_distance.min(dim=0)[0]
            sentence_distance = (sentence_distance - sentence_distance_min) / \
                                (sentence_distance_max - sentence_distance_min + 1e-6 * torch.ones_like(
                                    sentence_distance_max))

            weight = (torch.ones_like(sentence_distance) - sentence_distance) ** self.params.exp_param
            weight = torch.softmax(weight, dim=-1)

            # loss
            (scores, _, total_loss), original_loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y,
                                                                  masked_y=masked_y, get_scores=False)

            pred_mask = pred_mask[:-1].cuda().float()
            loss = (total_loss * token_weight * pred_mask).sum(dim=0)
            loss = weight.matmul(loss)

            # record loss
            self.stats[('AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(loss.item())
            self.stats[('Original_AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(
                original_loss.item())


        else:
            (scores, _, _), loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, masked_y=masked_y,
                                                get_scores=False)
            # just record the original loss
            self.stats[('AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(loss.item())
    ...
    def compute_similarity_score(self, fake_src_enc, tgt_enc):
        # normalize the hidden state
        norm_fake_src_enc = F.normalize(fake_src_enc, p=2, dim=-1)   # (bs, len2, hidden)
        norm_tgt_enc = F.normalize(tgt_enc, p=2, dim=-1)             # (bs, len1, hidden)

        # compute dot product
        token_cosine_scores = norm_tgt_enc.bmm(norm_fake_src_enc.transpose(1, 2))
        token_cosine_scores = token_cosine_scores.max(dim=2)[0]

        return token_cosine_scores
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        # f1 = open(params.competence_type + '_ende_' + params.diff_type + '.bt.src.len', 'a', encoding='utf-8')
        # f2 = open(params.competence_type + '_ende_' + params.diff_type + '.bt.src.sentences', 'a', encoding='utf-8')
        ...
        with torch.no_grad():
            ...
            enc1 = enc1.transpose(0, 1)    # (bs, len1, hidden)
            x2, len2, _ = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5))

            langs2 = x2.clone().fill_(lang2_id)

            if self.params.bt_sp == True:
                # encode fake target sentence and get the embedding
                fake_enc2 = _encoder('fwd', x=x2, lengths=len2, langs=langs2, causal=False)
                fake_enc2 = fake_enc2.transpose(0, 1)  # (bs, len2, hidden)

                # compute similarity
                token_cosine = self.compute_similarity_score(fake_enc2, enc1).transpose(0, 1)  # (len1, bs)
                sentence_cosine = torch.cosine_similarity(fake_enc2.mean(dim=1), enc1.mean(dim=1))

                del fake_enc2
            else:
                token_cosine = None
                sentence_cosine = None
        ...
        weight_mask = alen[:, None] >= len1[None] - 1  # used for the pad token-level weight
        y1 = x1[1:]
        masked_y1 = x1[1:].masked_select(pred_mask[:-1])
        ...
        if self.params.bt_sp == True:
            # loss
            (scores, _, total_loss), original_loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1,
                                                                  masked_y=masked_y1, get_scores=False)
            if self.params.bt_word == True:
                token_cosine = token_cosine[:-1, :].contiguous()
                weight_mask = weight_mask[:-1, :].contiguous()

                # normalize token cosine
                token_cosine_max = token_cosine.max(dim=0)[0]
                token_cosine_min = token_cosine.min(dim=0)[0]
                token_cosine = (token_cosine - token_cosine_min) / \
                               (token_cosine_max - token_cosine_min + 1e-6 * torch.ones_like(token_cosine_max))

                token_cosine = token_cosine ** self.params.exp_param
                token_mask_weight = torch.softmax(token_cosine.masked_fill_(mask=weight_mask, value=float("-inf")),
                                                  dim=0)
                token_loss = (total_loss * token_mask_weight).sum(dim=0)
            else:
                weight_mask = weight_mask[:-1, :].contiguous()
                # for the pad positions, just replace 0.0 for their loss values
                total_loss.masked_fill_(mask=weight_mask, value=0.0)
                token_loss = total_loss.sum(dim=0) / (len1 - torch.ones_like(len1))

            # normalize sentence cosine
            if self.params.bt_sentence == True:
                sentence_cosine_max = sentence_cosine.max(dim=0)[0]
                sentence_cosine_min = sentence_cosine.min(dim=0)[0]
                sentence_cosine = (sentence_cosine - sentence_cosine_min) / \
                                  (sentence_cosine_max - sentence_cosine_min + 1e-6 * torch.ones_like(
                                      sentence_cosine_max))

                sentence_cosine = sentence_cosine ** self.params.exp_param
                sentence_weight = torch.softmax(sentence_cosine, dim=0)

                loss = sentence_weight.matmul(token_loss)
            else:
                loss = token_loss.mean(dim=0)

            self.stats[('BT-%s-%s-%s' % (lang1, lang2, lang3))].append(loss.item())
            self.stats[('Original_BT-%s-%s-%s' % (lang1, lang2, lang3))].append(original_loss.item())

        else:
            (scores, _, _), loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, masked_y=masked_y1,
                                                get_scores=False)
