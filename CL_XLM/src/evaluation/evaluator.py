...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            # loss
            (_, word_scores, total_loss), loss = decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y,
                                                          masked_y=masked_y, get_scores=True)

            # process word_scores
            n_words += masked_y.size(0)
            xe_loss += loss.item() * len(masked_y)
            n_valid += (word_scores.max(1)[1] == masked_y).sum().item()
            if eval_memory:
                for k, v in self.memory_list:
                    all_mem_att[k].append((v.last_indices, v.last_scores))
        ...
        if eval_bleu:
            ...
            if lang1 == 'zh' and lang2 == 'en':
                ref_name = 'ref.{0}-{1}.{2}.*xt'.format(lang1, lang2, data_set)
