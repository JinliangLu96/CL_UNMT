...
class PredLayer(nn.Module):
    ...
    def forward(self, x, y, masked_x, masked_y, get_scores=False):
        """
        Compute the loss, and optionally the scores.
        """
...
class TransformerModel(nn.Module):
    ...
    #     return scores, loss
    def predict(self, tensor, pred_mask, y, masked_y, get_scores):
        """
        Given the last hidden state, compute word scores and/or the loss.
            `pred_mask` is a ByteTensor of shape (slen, bs), filled with 1 when
                we need to predict a word
            `y` is a LongTensor of shape (slen-1, bs)
            `get_scores` is a boolean specifying whether we need to return scores
        """
        masked_tensor = tensor[pred_mask.unsqueeze(-1).expand_as(tensor)].view(-1, self.dim)
        # scores, loss = self.pred_layer(masked_tensor, y, pred_mask, get_scores)
        scores, loss = self.pred_layer(tensor[:-1, :, :].contiguous(), y, masked_tensor, masked_y, get_scores)
        return scores, loss
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        # probability scores
        prob_scores = torch.zeros((bs,)).cuda()
        ...
        while cur_len < max_len:
            ...
            prob_scores += F.softmax(scores, dim=1).max(dim=1, keepdim=False)[0]  # (bs,)
        ...
        # compute uncertainty
        # length_ratio = torch.ones_like(src_len.float()) - src_len.float() / gen_len.float()
        # myzeros = torch.zeros_like(src_len.float())
        # length_ratio = torch.where(length_ratio > 0.0, myzeros, length_ratio)
        # uncertainty = (torch.ones_like(prob_scores) - (prob_scores / gen_len)) * torch.exp(length_ratio)
        uncertainty = torch.ones_like(prob_scores) - (prob_scores / gen_len)
        # normalize the uncertainty
        norm_uncertainty = F.softmax(uncertainty, dim=0)

        return generated[:cur_len], gen_len, norm_uncertainty
