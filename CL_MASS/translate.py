...
def get_parser():
    ...
    parser.add_argument("--reference_path", type=str, default="", help="Reference path")
    parser.add_argument("--bleu_path", type=str, default="", help="BLEU path")
...
def main(params):
    ...
    for i in range(0, len(src_sent), params.batch_size):
        ...
        if params.beam == 1:
            decoded, dec_lengths, _ = decoder.generate(encoded, lengths.cuda(), params.tgt_id, max_len=int(1.5 * lengths.max().item() + 10))
        else:
    ...
    result = eval_moses_bleu(params.reference_path, params.output_path, params.bleu_path)
    logger.info("Beam = %i, %s->%s BLEU score is %f. " % (params.beam, params.src_lang, params.tgt_lang, result))

def eval_moses_bleu(ref, hyp, bleu_path):
    """
    ...
    assert os.path.isfile(bleu_path)
    command = bleu_path + ' %s < %s'
